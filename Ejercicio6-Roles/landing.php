...
<body>
    ...
    <table border="1">
        ...
        foreach ($personas as $usuario) {
            ...
            <a href='#' class='editar' data-id='{$usuario['id_persona']}' data-nombre='{$usuario['nombre']}' data-apellido-paterno='{$usuario['apellidoPaterno']}' data-apellido-materno='{$usuario['apellidoMaterno']}' data-carnet='{$usuario['ci']}' data-celular='{$usuario['celular']}' data-tipo='{$usuario['tipo']}' data-saldo='{$usuario['saldo']}'>Editar</a>            
            </td>";
        }
        ?>
    </table>
    <script>
    function abrirModalUsuario(idUsuario, nombre = '', apellidoPaterno = '', apellidoMaterno = '', carnet = '', celular = '', tipoCuenta='', saldo='', crearUsuario = false) {
        ...
        if (crearUsuario) {
            document.getElementById('modalEditar').querySelector('form').reset(); // Resetear el formulario para crear un nuevo usuario
    ...
    enlacesEditar.forEach(function(enlace) {
        enlace.addEventListener('click', function(event) {
            ...
            abrirModalUsuario(idUsuario, nombre, apellidoPaterno, apellidoMaterno, carnet, celular, tipoCuenta, saldo); 
        });
    });
